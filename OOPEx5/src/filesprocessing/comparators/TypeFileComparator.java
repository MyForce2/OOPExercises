...
/**
 * A TypeFileComparator used to compare files by their type 
 * (alphabetically)
 * if the two files have the same file type, compares them 
 * using the AbsFileComparator
...
public class TypeFileComparator implements Comparator<File> {
	...
	public int compare(File f1, File f2) {
		...
		if(result == 0) {
			// In the case file types are equal, compare 
			// the files using the AbsFileComparator
	 ...
	 * @return The file type of the given file, the 
	 * NO_TYPE field if no type is specified
	 */
