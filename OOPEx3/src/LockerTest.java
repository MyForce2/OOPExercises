...
public class LockerTest {
	...
	public void testRemoveItem() {
		...
		for(int i = 0; i < items.length; i++) 
			locker.removeItem(items[i], locker.getItemCount(items[i].getType()));
		lts.resetInventory();
		assertTrue("Locker class remove item doesn't work with null values", locker.removeItem(null, 1) == Locker.OPEARATION_FAILED_ERROR);
		assertTrue("Locker class remove item doesn't work with 0 values", locker.removeItem(items[0], 0) == Locker.OPERATION_SUCCESSFUL);
		assertTrue("Locker class remove item doesn't work with negative values", locker.removeItem(items[0], -1) == Locker.OPEARATION_FAILED_ERROR);
		locker.removeItem(items[0], locker.getItemCount(items[0].getType()));
		assertTrue("Locker class remove item doesn't work values", locker.removeItem(items[0], 1) == Locker.OPEARATION_FAILED_ERROR);
		for(int i = 0; i < items.length; i++) 
			locker.removeItem(items[i], locker.getItemCount(items[i].getType()));
	}
